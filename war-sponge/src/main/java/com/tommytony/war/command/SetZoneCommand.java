...
public class SetZoneCommand implements CommandCallable {
    ...
    public CommandResult process(CommandSource commandSource, String s) throws CommandException {
        ...
        if (commandSource instanceof Player) {
            ...
            commandSource.sendMessage(Text.of("This command cannot be used from console."));
        ...
        if (plugin.getState(player).isCreatingZone() && (args[0].equalsIgnoreCase("c1") || args[0].equalsIgnoreCase("c2"))) {
            ...
            if (args[0].equalsIgnoreCase("c1")) {
                ...
                commandSource.sendMessage(Text.of(MessageFormat.format("Set position 1 of warzone {0} to {1}.", state.getZoneName(), state.getPosition1().toString())));
                return CommandResult.success();
            } else if (args[0].equalsIgnoreCase("c2")) {
                if (!state.isPosition1Set()) {
                    throw new CommandException(Text.of("Please place corner 1 of the zone before placing corner 2."));
                }
                ...
                if (plugin.getValidator().validateDimensions(cuboid) == ZoneValidator.ValidationStatus.INVALID) {
                    throw new CommandException(Text.of(TextColors.RED, MessageFormat.format("Failed to create warzone {0} with dimensions {1}. Make your zone larger/smaller by placing your second corner somewhere else.",
                            state.getZoneName(), cuboid.toString())));
                }
                commandSource.sendMessage(Text.of(MessageFormat.format("Set position 2 of warzone {0} to {1}.", state.getZoneName(), pos2.toString())));
        ...
        if (args[0].equalsIgnoreCase("c1") || args[0].equalsIgnoreCase("c2")) {
            throw new CommandException(Text.of("Create a new warzone before setting the corners."));
        }
    ...
    public Optional<? extends Text> getShortDescription(CommandSource commandSource) {
        return Optional.of(Text.of("Creates a warzone"));
    }

    @Override
    public Optional<? extends Text> getHelp(CommandSource commandSource) {
        return Optional.of(Text.of("Creates a warzone. Puts the player into zone setting mode, which then allows them to select blocks."));
    }

    @Override
    public Text getUsage(CommandSource commandSource) {
        return Text.of("<name / c1 / c2>");
    }
}
